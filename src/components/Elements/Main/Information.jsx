...
const Information = ({movie}) => {
	const addToFavorites = (movieName) => {
		let favorites = localStorage.getItem('favMovies') //получаем данные по ключу 

		if (favorites) {
			favorites = JSON.parse(favorites)
			localStorage.setItem('favMovies', [...favorites, movieName]) //сохраняем пару ключ/значение
	...
	return (
		<div className={styles.info}>
			<img
			 src={movie.logo} alt={movie.name} width='200' style={{ opacity: 0.7}} />
